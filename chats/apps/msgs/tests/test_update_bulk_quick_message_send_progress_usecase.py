...
class UpdateBulkQuickMessageSendProgressUseCaseTests(TestCase):
    def setUp(self):
        ...
        self.usecase = UpdateBulkQuickMessageSendProgressUseCase()
    ...
    def _create_bulk_message(self, status):
        return BulkQuickMessageSendMessage.objects.create(
            ...
            errors=(
                {"error": "x"}
                if status == BulkQuickMessageSendMessageStatus.FAILED
                else None
            ),
        )
    ...
    def test_aggregates_counts_and_sends_ws_progress(self, mock_send):
        self._create_bulk_message(BulkQuickMessageSendMessageStatus.SUCCESS)
        self._create_bulk_message(BulkQuickMessageSendMessageStatus.SUCCESS)
        self._create_bulk_message(BulkQuickMessageSendMessageStatus.FAILED)
