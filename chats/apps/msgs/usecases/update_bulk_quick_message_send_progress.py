...
class UpdateBulkQuickMessageSendProgressUseCase:
    """
    Aggregates per-room bulk quick-message send outcomes, marks the parent as
    FINISHED at 100%, and notifies the requesting user over WebSocket.
    """

    def execute(self, bulk_send_uuid: UUID) -> Optional[dict]:
        bulk_send = (
            BulkQuickMessageSend.objects.select_related("user", "project")
            .filter(uuid=bulk_send_uuid)
            .first()
        )
        if not bulk_send:
            logger.info(
                "[UpdateBulkQuickMessageSendProgressUseCase] BulkQuickMessageSend "
                "not found for UUID: %s",
                bulk_send_uuid,
            )
            return None

        stats = BulkQuickMessageSendMessage.objects.filter(
            bulk_quick_message_send=bulk_send
        ).aggregate(
            success_total=Count(
                "pk", filter=Q(status=BulkQuickMessageSendMessageStatus.SUCCESS)
            ),
            failed_total=Count(
                "pk", filter=Q(status=BulkQuickMessageSendMessageStatus.FAILED)
            ),
        )
        ...
        if not permission:
            logger.info(
                "[UpdateBulkQuickMessageSendProgressUseCase] No project permission "
                "for user %s on project %s; skipping WS progress update for bulk "
                "quick send %s",
                bulk_send.user_id,
        ...
        send_channels_group(
            ...
            call_type="notify",
            action="bulk_quick_message_progress_update",
            content=content,
        )
        return content
