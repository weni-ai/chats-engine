...
class ListQueuePermissionsTests(TestCase):
    def setUp(self):
        ...
        self.perm = ProjectPermission.objects.create(project=self.project, user=self.user, role=1)
        self.qa = QueueAuthorization.objects.create(permission=self.perm, queue=self.queue, role=1)
    ...
    def test_list_queue_permissions_returns_items(self, mock_cache):
        ...
        req = self.factory.get(f"/x?user_email=admin@acme.com&project={self.project.pk}")
        force_authenticate(req, user=self.user)
        resp = view(req)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.data["user_permissions"]), 1)

    @patch("chats.apps.api.v1.queues.viewsets.get_user_id_by_email_cached", return_value=None)
