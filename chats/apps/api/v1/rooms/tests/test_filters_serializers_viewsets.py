...
class RoomFilterTests(TestCase):
    ...
    @patch("chats.apps.api.v1.rooms.filters.get_user_id_by_email_cached", return_value=None)
    def test_filter_project_returns_none_when_email_unknown(self, _):
        request = self.factory.get(f"/x?email=unknown@acme.com")
        request.user = self.user
        qs = Room.objects.all()
        f = RoomFilter(data={"project": str(self.project.pk)}, queryset=qs, request=request)
        self.assertFalse(f.qs.exists())

class TransferRoomSerializerTests(TestCase):
    @patch("chats.apps.api.v1.rooms.serializers.get_user_id_by_email_cached", return_value=99)
...
class RoomViewsetBulkTransferTests(TestCase):
    def setUp(self):
        ...
        self.sector = self.project.sectors.create(name="S", rooms_limit=5, work_start="08:00", work_end="18:00")
        self.queue = self.sector.queues.create(name="Q")
        self.perm = ProjectPermission.objects.create(project=self.project, user=self.agent, role=1)
    ...
    def test_bulk_transfer_assigns_user_instance(self, mock_cache, _routing, _feedback):
        ...
        req = self.factory.patch(f"/x?user_email=agent@acme.com", data={"rooms_list": [str(self.room.uuid)]}, content_type="application/json")
    ...
    @patch("chats.apps.api.v1.rooms.viewsets.get_user_id_by_email_cached", return_value=None)
    def test_bulk_transfer_returns_400_when_user_not_found(self, _):
        view = RoomViewset.as_view({"patch": "bulk_transfer"})
        req = self.factory.patch(f"/x?user_email=no@acme.com", data={"rooms_list": [str(self.room.uuid)]}, content_type="application/json")
        force_authenticate(req, user=self.admin)
        resp = view(req)
