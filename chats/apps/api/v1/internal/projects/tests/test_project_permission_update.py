...
class ProjectPermissionUpdateTests(TestCase):
    ...
    def test_update_uses_user_id_email(self, mock_persist, mock_cache):
        ...
        req = self.factory.put(
            "/x",
            data={"user": "Agent@Acme.com", "role": 1, "project": str(self.project.pk)},
            content_type="application/json",
        )
        force_authenticate(req, user=self.admin)
        resp = view(req, uuid="ignored")
        self.assertEqual(resp.status_code, 200)
        perm = ProjectPermission.objects.get(
            project=self.project, user_id="agent@acme.com"
        )
        self.assertEqual(perm.role, 1)

    @patch(
        "chats.apps.api.v1.internal.projects.viewsets.get_user_id_by_email_cached",
        return_value=None,
    )
    @patch(
        "chats.apps.api.v1.internal.projects.viewsets.persist_keycloak_user_by_email"
    ...
    def test_update_404_when_user_missing(self, _persist, _cache):
        ...
        req = self.factory.put(
            "/x",
            data={"user": "x@acme.com", "role": 1, "project": str(self.project.pk)},
            content_type="application/json",
        )
        force_authenticate(req, user=self.admin)
        resp = view(req, uuid="ignored")
