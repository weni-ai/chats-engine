...
from chats.apps.sectors.utils import get_country_holidays, get_country_from_timezone
        ...
        try:
            ...
            for holiday_date_str in selected_holidays:
                try:
                    ...
                    if SectorHoliday.objects.filter(sector=sector, date=holiday_date).exists():
                    ...
                    errors.append(f"Error creating holiday for {holiday_date_str}: {str(e)}")
            
            return Response({
                "created": len(created_holidays),
                "holidays": created_holidays,
                "errors": errors
            }, status=status.HTTP_201_CREATED if created_holidays else status.HTTP_400_BAD_REQUEST)
