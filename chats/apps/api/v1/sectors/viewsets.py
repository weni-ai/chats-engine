...
class SectorHolidayViewSet(viewsets.ModelViewSet):
    ...
    def _get_official_holidays_by_year(self, sector, enabled_dates, disabled_dates):
        dates_to_touch = [
            *(self._parse_date_str(date) for date in enabled_dates),
            *(self._parse_date_str(date) for date in disabled_dates),
        ]
        years = {d.year for d in dates_to_touch if d}
        country_code = get_country_from_timezone(str(sector.project.timezone))
        return {y: get_country_holidays(country_code, y) or {} for y in years}

    def _enable_holiday(self, sector, date, name):
        obj = SectorHoliday.objects.filter(sector=sector, date=date).first()
        if obj:
            self._update_existing_holiday(obj, name)
        else:
    ...
    def _update_existing_holiday(self, obj, name):
        updates = {}
        if obj.is_deleted:
    ...
    def _disable_holiday(self, sector, date):
        obj = SectorHoliday.objects.filter(sector=sector, date=date).first()
    ...
    def _handle_patch_holidays(self, request):
        ...
        official_by_year = self._get_official_holidays_by_year(
            sector, enabled_dates, disabled_dates
        )

        enabled_count, disabled_count, errors = self._process_holiday_changes(
            sector, enabled_dates, disabled_dates, official_by_year
        )

        return Response(
            {"enabled": enabled_count, "disabled": disabled_count, "errors": errors},
            status=status.HTTP_200_OK,
        )

    def _process_holiday_changes(
        self, sector, enabled_dates, disabled_dates, official_by_year
    ):
        enabled_count = 0
        disabled_count = 0
        errors = []
        ...
        for ds in disabled_dates:
            ...
            if not d:
                errors.append(f"Invalid date format: {ds}")
                continue
            if self._disable_holiday(sector, d):
                disabled_count += 1

        return enabled_count, disabled_count, errors

    def _handle_get_holidays(self, request):
        project_uuid = request.query_params.get("project")
        ...
        year = self._parse_year_param(year_param, project)
        if isinstance(year, Response):
            return year
    ...
    def _parse_year_param(self, year_param, project):
        if not year_param:
    ...
    def official_holidays(self, request):
        ...
        if request.method.upper() == "PATCH":
            return self._handle_patch_holidays(request)
        return self._handle_get_holidays(request)
