...
class Metrics():
    ...
    def metric_total_rooms_created_last_month(self):
        ...
        chats_total_rooms_last_month.labels("total_rooms_last_month").set(last_month_rooms_count)

    def metric_total_rooms_created_last_3_months(self):
        now = timezone.now()
        last_3_months = now + relativedelta(months=-3)

        last_3_months_rooms_count = Room.objects.filter(created_on__gte=last_3_months).count()
        chats_total_rooms_last_3_months.labels("total_rooms_last_3_months").set(last_3_months_rooms_count)

    def metric_total_rooms_created_last_6_months(self):
        now = timezone.now()
        last_6_months = now + relativedelta(months=-6)

        last_6_months_rooms_count = Room.objects.filter(created_on__gte=last_6_months).count()
        chats_total_rooms_last_6_months.labels("total_rooms_last_6_months").set(last_6_months_rooms_count)
    ...
    def metric_total_rooms_created_last_year(self):
        ...
        chats_total_rooms_last_year.labels("total_rooms_last_year").set(last_year_rooms_count)
