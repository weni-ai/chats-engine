...
class TestRoomHistoryEndpoint(TestCase):
    fixtures = ["chats/fixtures/fixture_app.json"]  # Use a fixture que contém dados de projeto
    ...
    def setUp(self):
        ...
        # Obtenha uma fila existente da fixture
        ...
        # Use o token da fixture que sabemos que funciona
        self.client.credentials(HTTP_AUTHORIZATION="Bearer f3ce543e-d77e-4508-9140-15c95752a380")
    ...
    def test_create_history_success(self):
        ...
        # Verificamos apenas os textos e horários, não a direção
        message_texts = {msg.text: msg.created_on.isoformat() for msg in messages}
        self.assertEqual(
            message_texts["test message"],
            "2025-03-24T20:22:52.067586+00:00"
        )
        self.assertEqual(
            message_texts["resposta"],
            "2025-03-24T20:22:56.428391+00:00"
        )
    ...
    def test_create_history_with_attachments(self):
        ...
        response = self.client.post(
            f'/v1/external/rooms/{self.room.pk}/history/',  # Use pk, não uuid
            data=messages_data,
            format='json'
        )
        ...
        self.assertEqual(message.created_on.isoformat(), "2025-03-24T20:22:52.067586+00:00")
    ...
    def test_create_history_wrong_project(self):
        ...
        # Usa outro token que existe na fixture mas não tem permissão neste projeto
        self.client.credentials(HTTP_AUTHORIZATION="Bearer 5e2add2a-30c3-499e-b9c4-15be8107130e")
    ...
    def test_create_history_room_not_found(self):
        ...
        response = self.client.post(
            f'/v1/external/rooms/{fake_id}/history/',  # Use pk numérico, não uuid
            data=[self.base_message],
            format='json'
        )
    ...
    def test_create_history_invalid_data(self):
        ...
        invalid_data = [{
            "direction": "incoming",
            "text": None,  # Explicitamente None
            "attachments": [],  # Lista vazia
            "created_on": "2025-03-24T20:22:52.067586Z"
        }]
    ...
    def test_create_history_triggers_default_message(self):
        ...
        self.assertEqual(response.status_code, 201)
        self.assertEqual(Message.objects.filter(room=self.room).count(), 1)  # ou 2 se a mensagem padrão for criada
