...
class BaseHMACSignatureAuthentication(BaseAuthentication):
    """
    Base class for the signature of the request.
    """

    _secret_setting_key = None

    def _get_secret_key(self):
        if self._secret_setting_key is None:
            raise ImproperlyConfigured("Secret key not set")

        return getattr(settings, self._secret_setting_key)
