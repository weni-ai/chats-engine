...
def _generate_xlsx_from_report_data(report_data):
    output = io.BytesIO()
...
def _generate_csv_from_report_data(report_data):
    zip_buf = io.BytesIO()
...
def _save_custom_report_locally(output, project, file_type):
    dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
    base_dir = getattr(
        settings, "REPORTS_SAVE_DIR", os.path.join(settings.MEDIA_ROOT, "reports")
    )
...
def generate_custom_fields_report(
    ...
    try:
        ...
        if file_type == "xlsx":
            output = _generate_xlsx_from_report_data(report_data)
        else:
            output = _generate_csv_from_report_data(report_data)

        dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
        if getattr(settings, "REPORTS_SAVE_LOCALLY", True):
            dt = _save_custom_report_locally(output, project, file_type)

        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_report_email(
                    output, project, file_type, user_email, report_status, dt
                )
            except Exception as e:
        ...
        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_error_email(project, user_email, report_status, str(e))
...
def _write_xlsx_queryset(writer, sheet_name, qs, chunk_size):
    total = qs.count()
    row_offset = 0
...
def _write_csv_queryset(csv_buffers, sheet_name, qs):
    total = qs.count()
    row_offset = 0
...
def _process_xlsx_model(writer, view, model_name, model_cfg, project, available_fields):
    ...
    if qs is not None and qs.count() > 0:
        _write_xlsx_queryset(writer, model_name, qs, chunk_size)
    elif qs is not None:
...
def _generate_xlsx_report(view, fields_config, project, available_fields):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        for model_name in ["rooms", "agent_status_logs"]:
            model_cfg = (fields_config or {}).get(model_name) or {}
            _process_xlsx_model(
                writer, view, model_name, model_cfg, project, available_fields
            )
    return output


def _generate_csv_report(view, fields_config, project, available_fields):
    csv_buffers = {}
    ...
    for model_name in ["rooms", "agent_status_logs"]:
        if model_name in (fields_config or {}) and model_name in available_fields:
            ...
            if "queryset" in (query_data or {}):
                _write_csv_queryset(csv_buffers, model_name, query_data["queryset"])
...
def _save_report_locally(output, project, file_type, report):
    dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
    base_dir = getattr(
        settings, "REPORTS_SAVE_DIR", os.path.join(settings.MEDIA_ROOT, "reports")
    )
...
def _send_report_email(output, project, file_type, user_email, report, dt):
    ...
    logging.info(
        "Report email sent successfully to %s | report_uuid=%s", user_email, report.uuid
    )


def _send_error_email(project, user_email, report, error_message):
    ...
    message_plain, message_html = get_report_failed_email(project.name, error_message)
    ...
    logging.info(
        "Error notification sent to %s | report_uuid=%s", user_email, report.uuid
    )
...
def process_pending_reports():
    ...
    try:
        ...
        if file_type == "xlsx":
            output = _generate_xlsx_report(
                view, fields_config, project, available_fields
            )
        else:
            output = _generate_csv_report(
                view, fields_config, project, available_fields
            )

        dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
        if getattr(settings, "REPORTS_SAVE_LOCALLY", True):
            dt = _save_report_locally(output, project, file_type, report)
        ...
        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_report_email(output, project, file_type, user_email, report, dt)
            except Exception as e:
        ...
        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_error_email(project, user_email, report, str(e))
            except Exception as email_error:
                logging.exception(
                    "Error sending error notification email: %s", email_error
                )
