...
def _get_chunk_size(total_records):
...
def _write_xlsx_chunks(writer, sheet_name, qs, chunk_size):
    total = qs.count()
    row_offset = 0
    for start in range(0, total, chunk_size):
        ...
        df.to_excel(writer, sheet_name=sheet_name[:31], index=False, header=(row_offset == 0), startrow=row_offset if row_offset > 0 else 0)
        row_offset += len(df)


def _write_csv_chunks(buf, qs, chunk_size):
    total = qs.count()
    row_offset = 0
    for start in range(0, total, chunk_size):
        end = min(start + chunk_size, total)
        logging.info("Writing chunk (csv): start=%s end=%s total=%s", start, end, total)
        chunk = _strip_tz(list(qs[start:end]))
        if not chunk:
            continue
        df = _excel_safe_dataframe(pd.DataFrame(chunk))
...
def _process_xlsx_model(writer, model_name, model_cfg, view, project, available_fields):
    ...
    if qs is not None and qs.count() > 0:
        _write_xlsx_chunks(writer, model_name, qs, chunk_size)
    else:
        requested_fields = model_cfg.get("fields") or []
        pd.DataFrame(columns=requested_fields).to_excel(writer, sheet_name=model_name[:31], index=False)
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        for model_name in ["rooms", "agent_status_logs"]:
            ...
            if not model_cfg or model_name not in available_fields:
                continue
            _process_xlsx_model(writer, model_name, model_cfg, view, project, available_fields)


def _process_csv_report(view, fields_config, project, available_fields):
    csv_buffers = {}
    for model_name in ["rooms", "agent_status_logs"]:
        if model_name not in (fields_config or {}) or model_name not in available_fields:
            continue
        query_data = view._process_model_fields(model_name, fields_config[model_name], project, available_fields)
        if "queryset" not in (query_data or {}):
            continue
        qs = query_data["queryset"]
        buf = io.StringIO()
        csv_buffers[model_name] = buf
        chunk_size = _get_chunk_size(qs.count())
        _write_csv_chunks(buf, qs, chunk_size)
...
def _save_report_locally(output, project, file_type, report_uuid):
    base_dir = getattr(settings, "REPORTS_SAVE_DIR", os.path.join(settings.MEDIA_ROOT, "reports"))
    os.makedirs(base_dir, exist_ok=True)
    dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
    ext = "xlsx" if file_type == "xlsx" else "zip"
    filename = f"custom_report_{project.uuid}_{dt}.{ext}".replace("/", "-").replace("\\", "-")
    output.seek(0)
    file_path = os.path.join(base_dir, filename)
    with open(file_path, "wb") as f:
        f.write(output.getvalue())
    logging.info("Custom report saved at: %s | report_uuid=%s", file_path, report_uuid)


def _send_report_email(output, project, file_type, user_email, report_uuid):
    from chats.core.storages import ReportsStorage
    storage = ReportsStorage()
    dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
    ext = "xlsx" if file_type == "xlsx" else "zip"
    ...
    logging.info("Report uploaded to S3: %s | report_uuid=%s | url=%s", file_path, report_uuid, download_url)

    subject = f"Custom report for the project {project.name} - {dt}"
    message_plain, message_html = get_report_ready_email(project.name, download_url)
    email = EmailMultiAlternatives(subject=subject, body=message_plain, from_email=settings.DEFAULT_FROM_EMAIL, to=[user_email])
    email.attach_alternative(message_html, "text/html")
    email.extra_headers = {"X-No-Track": "True", "X-Track-Click": "no", "o:tracking-clicks": "no"}
    email.send(fail_silently=False)
    logging.info("Report email sent successfully to %s | report_uuid=%s", user_email, report_uuid)


def _send_error_email(project, user_email, error, report_uuid):
    from chats.apps.dashboard.email_templates import get_report_failed_email
    dt = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
    subject = f"Error generating custom report for project {project.name} - {dt}"
    message_plain, message_html = get_report_failed_email(project.name, str(error))
    email = EmailMultiAlternatives(subject=subject, body=message_plain, from_email=settings.DEFAULT_FROM_EMAIL, to=[user_email])
    email.attach_alternative(message_html, "text/html")
    email.send(fail_silently=False)
    logging.info("Error notification sent to %s | report_uuid=%s", user_email, report_uuid)
...
def process_pending_reports():
    ...
    try:
        ...
        if getattr(settings, "REPORTS_SAVE_LOCALLY", True):
            _save_report_locally(output, project, file_type, report.uuid)

        logging.info("Processing report %s for project %s done.", report.uuid, project.uuid)

        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_report_email(output, project, file_type, user_email, report.uuid)
            except Exception as e:
        ...
        if getattr(settings, "REPORTS_SEND_EMAILS", False):
            try:
                _send_error_email(project, user_email, e, report.uuid)
            except Exception as email_error:
