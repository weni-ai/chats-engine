...
class CheckModulePermissionTests(SimpleTestCase):
    ...
    @mock.patch("chats.apps.accounts.authentication.drf.backends.ContentType.objects.get_for_model")
    @mock.patch("chats.apps.accounts.authentication.drf.backends.Permission.objects.get_or_create")
    def test_check_module_permission_grants_permission(self, mock_get_or_create, mock_get_for_model):
...
class OIDCBackendTests(SimpleTestCase):
    ...
    def test_create_user_sanitises_username(self, mock_get_username):
        ...
        dummy_user_instance = SimpleNamespace(first_name="", last_name="", save=lambda: None)
        dummy_user_model.objects.get_or_create.return_value = (dummy_user_instance, True)
        ...
        with mock.patch.object(sut, "User", dummy_user_model):
            ...
            dummy_user_model.objects.get_or_create.assert_called_once_with(email="john@weni.io")